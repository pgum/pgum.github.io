				...
				if (!initialised) {
					...
					if (custom) {
						custom.forEach(function(element, index, array) {
							...
							addToolbarButton(element.title, element.href , element.icon, element.style, function(){location.href=element.href;})
						});
					}
					...
					// PJG Custom
					//
					function generateCustom(name, location) {
						var item = create('li', { class: 'slide-menu-item external active selected' });
						item.appendChild(create('i', {class: 'far fa-arrow-alt-circle-right fa-fw future'}));
						item.appendChild(create('a', {href: location, class: 'slide-menu-item-title'}, name));
						return item;
					}
					...
					function createSlideMenu() {
						if ( !document.querySelector('section[data-markdown]:not([data-markdown-parsed])') ) {
							...
							for(let c in customFields){items.appendChild(generateCustom(customFields[c].title, customFields[c].href));}
